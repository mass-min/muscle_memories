<form action="{{ route('workouts.store', null, false) }}" method="POST">
    ...
    <div class="grid grid-cols-5 gap-4">
        <div>
            <x-form-label for="training[0][menu]">トレーニングメニュー</x-form-label>
        ...
        <div class="col-span-3">
            <div class="grid grid-cols-3">
                <div>
                    <x-form-label for="training[0][sets][0][weight]">重量(kg)</x-form-label>
                    <x-number-form id="weight" type="number" name="training[0][sets][0][weight]" required />
                </div>

                <div>
                    <x-form-label for="training[0][sets][0][reps]">レップ数(回)</x-form-label>
                    <x-number-form id="reps" type="number" name="training[0][sets][0][reps]" required />
                </div>

                <div>
                    <x-form-label for="training[0][sets][0][interval_seconds]">インターバル時間(秒)</x-form-label>
            ...
            <div class="grid grid-cols-3">
                <div>
                    <x-form-label for="training[0][sets][1][weight]">重量(kg)</x-form-label>
                    <x-number-form id="weight" type="number" name="training[0][sets][1][weight]" required />
                </div>

                <div>
                    <x-form-label for="training[0][sets][1][reps]">レップ数(回)</x-form-label>
                    <x-number-form id="reps" type="number" name="training[0][sets][1][reps]" required />
                </div>

                <div>
                    <x-form-label for="training[0][sets][1][interval_seconds]">インターバル時間(秒)</x-form-label>
